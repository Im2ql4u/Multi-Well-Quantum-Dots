#!/usr/bin/env bash
# Launch improved no-ref two-stage runs for N=3 and N=4 across 6 GPUs.
# Strategy: improved_self_residual — wider stratified sampler, no backflow, pure variance.
# Stage A: 4000 epochs (longer variance-minimization to escape localized basin)
# Stage B: uses base config epochs (4000 for N=3, 5000 for N=4)
# Seeds: 42, 314, 901
#
# GPU assignments:
#   GPU0: N=3 seed=42
...
LOG_DIR="$REPO/logs/improved_noref_n3n4"
mkdir -p "$LOG_DIR"

echo "=== Launching improved no-ref N=3/N=4 campaign at $(date) ==="
